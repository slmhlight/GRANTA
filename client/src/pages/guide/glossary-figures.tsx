...
const CAPTIONS: Record<string, string> = {
  ...
  'polymer-pyramid':
    '그림 · 폴리머 피라미드(개략). 아래로 갈수록 값싸고 많이 쓰는 범용 수지, 위로 갈수록 비싸고 강한 소수 정예다 — 연속사용온도 150 °C 가 고성능(슈퍼 엔지니어링) 폴리머의 문턱이며, 반결정(내약품·내마모)과 비정질(투명·치수안정)의 두 갈래가 각 층을 관통한다.',
  'ceramic-families':
    '그림 · 세라믹 가족 지도(개략). 세라믹은 단단할수록 잘 깨지는 저울 위에 있다 — B₄C·SiC 는 경도의 극단, 지르코니아(Y-TZP·Mg-PSZ)는 변태강화로 인성의 극단을 차지하고, 알루미나는 그 중간의 가성비 표준이다. 강(K_IC 50+)과의 거리가 세라믹 설계가 결함 관리에 집착하는 이유다.',
  'bainite-upper-lower':
