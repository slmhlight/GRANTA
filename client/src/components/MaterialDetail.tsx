                      ...
                      <div className="space-y-1 text-[12px] mt-2 pt-2 border-t border-current/15">
                        ...
                        <div className="mt-2 pt-2 border-t-2 border-current/30">
                          ...
                          {/* R226k — 합금별 용접 권고: m.profiles.wg → welding-guidance.json (구 인라인 71블록 이관; nonferrous 는 CE 지표 없을 때만).
                              W20 — 텍스트는 계열 공통 + 이 재료 grade 전용(by_grade) 이 이미 합쳐져 형제 grade 는 표시되지 않음. */}
                          {weldGuidance && (
                            <div className="mt-2">
                              <RecText className="text-[11px] leading-relaxed">{weldGuidance}</RecText>
                            </div>
                          )}
              ...
              return (
                <details className="rounded-lg border-2 border-cyan-300 bg-cyan-50/40 p-3" open={isSecOpen('corr')} onToggle={(e) => toggleSec('corr', e.currentTarget.open)}>
                  ...
                  <div className="mt-2 pt-2 border-t border-cyan-600/15">
                    ...
                    {/* H6 E15c/E15f/E15h — 합금 개별 노트(텍스트가 합금 지정으로 시작 — 라벨 불요) + 노트별 출처 */}
                    {cp.alloyNote && (
                      <p className="text-[10.5px] leading-snug mb-1.5 rounded bg-cyan-600/10 border border-cyan-600/25 px-1.5 py-1">
                        <span className="text-foreground/85">{cp.alloyNote.t}</span>{' '}
