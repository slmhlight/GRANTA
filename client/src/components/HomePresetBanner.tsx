    ...
    <div className="flex items-center gap-1.5 sm:gap-2 px-2 sm:px-4 py-1 sm:py-1.5 bg-amber-500/10 border-b border-amber-500/30 text-[11px] sm:text-xs">
      ...
      <div className="ml-auto flex items-center gap-1 flex-shrink-0">
        ...
        {viewFiltered.length > 0 && !selectedMaterial && (
          <button onClick={() => setSelectedMaterial(viewFiltered[0])} className="hidden sm:inline-flex text-[11px] px-2 py-0.5 rounded border border-emerald-500/60 bg-emerald-500/10 text-emerald-700 hover:bg-emerald-500/20 items-center gap-0.5" title={t('banner.firstCandidateTip')}>
            <Info className="w-3 h-3" /> {t('banner.firstCandidate')} ({viewFiltered.length})
          </button>
        )}
        {compareList.length >= 2 && !showCompare && (
          <button onClick={() => setShowCompare(true)} className="hidden sm:inline-flex text-[11px] px-2 py-0.5 rounded border border-sky-500/60 bg-sky-500/10 text-sky-700 hover:bg-sky-500/20 items-center gap-0.5" title={t('banner.openCompareTip')}>
        ...
        {appliedPreset.suggestedView && viewMode !== appliedPreset.suggestedView && (
          <button onClick={() => setViewMode(appliedPreset.suggestedView!)} className="text-[10px] sm:text-[11px] px-1.5 sm:px-2 py-0.5 rounded border border-amber-500/60 bg-amber-500/15 text-amber-800 hover:bg-amber-500/25 flex items-center gap-0.5" title={t('banner.switchViewTip')}>
