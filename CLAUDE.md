...
## Data pipeline
- **자료 추가·교정 워크플로우** → [`docs/DATA-WORKFLOW.md`](docs/DATA-WORKFLOW.md) (어느 파일을 편집할지 — 4550-LOC build-materials 회피). R226e 보강: 순수함수 lib 추출(`scripts/lib/anomalies.mjs`·`source-labels.mjs`, 중복 제거) · CI 게이트 테스트(`registry-integrity`·`corrections-schema`·`golden-values`) · 출처 `authority` 등급(standard>handbook>manufacturer>aggregator>other) · verify:urls 안티봇 자동 후보 검출.
