...
- **1,124 materials** total — live counts in `client/public/build-meta.json` (SSOT). By category: 918 Metal · 133 Polymer · 39 Ceramic · 34 Composite. (R226b: 1198→1107; R226c: +7 cast → 1114; R226d: −2 dup → 1112; R226j: +1 ADC12 → 1113; **R226l: +11 Ansys Granta MI datasheet entry** (`data/granta-datasheets.json` — 1015/1020/1022 조건·5160/6150·Monel 400 hard) → 1124.)
- **R226l Granta 파이프**: 사용자 제공 Granta MI PDF → RT 값 검증(교정 5건: PEEK GF30 El·1015/1020 annealed·6150 El·Monel hard 복원) + **온도곡선 벡터 추출**(승인 — 결정적 좌표+23°C 앵커 검증) → `data/elevated-temp-curves.json` **by_id**(stable_id 키, build:data 가 부착, 14 entry). 신규 entry 는 granta-datasheets.json(supRaw 합류). 유사재료 카드에 인사이트 융합(`decisionContext` — 후보 전용 when-to-use + 절삭성 델타, ID 조회).
