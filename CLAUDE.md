...
## Data pipeline
- **REGEX→Material ID 전환 계획** → [`docs/REGEX-TO-ID-MIGRATION.md`](docs/REGEX-TO-ID-MIGRATION.md) (런타임 분류 regex 는 R226j/k 로 소거 완료; 빌드 파이프라인 name-regex override(R199 등)는 Phase1~6 로 ID화 예정 — 백로그 A9). **자료 추가·교정 워크플로우** → [`docs/DATA-WORKFLOW.md`](docs/DATA-WORKFLOW.md) (어느 파일을 편집할지 — 4550-LOC build-materials 회피). **데이터 발전 전략(분기 로드맵)** → [`docs/DATA-STRATEGY.md`](docs/DATA-STRATEGY.md). **장기 운영 원칙·유지보수 프로토콜** → [`docs/LONGTERM-PLAN.md`](docs/LONGTERM-PLAN.md) (백본 판정: 대규모 리팩터 불필요 — 동결+게이트 봉쇄; 확장은 §3 파이프 맵). **전량 작업 인벤토리(착수·수용 기준+리스크)** → [`docs/MASTER-BACKLOG.md`](docs/MASTER-BACKLOG.md) — 새 작업은 여기 등재 후 착수, 커밋에 백로그 ID 인용. R226e 보강: 순수함수 lib 추출(`scripts/lib/anomalies.mjs`·`source-labels.mjs`, 중복 제거) · CI 게이트 테스트(`registry-integrity`·`corrections-schema`·`golden-values`) · 출처 `authority` 등급(standard>handbook>manufacturer>aggregator>other) · verify:urls 안티봇 자동 후보 검출 · **points↔ranges 정합**(build-registry 4d — 상류 range override 후 stale 였던 99 entry points 재생성, 표=Ashby 일치; registry-integrity 가 게이트).
...
- **1,142 materials** total — live counts in `client/public/build-meta.json` (SSOT). By category: 936 Metal · 133 Polymer · 39 Ceramic · 34 Composite. (R227/H4d~e: +4 백주철·ADI +1 Grade P92 +8 absent-triage(HK40·ZA-27·DC53·M35·A357.0·A201.0·IN-100·740H) (`data/alloy-additions.json` — supRaw **최후미 합류·지정 append 지점**, 규격/제조사 datasheet 검증값) → 1142. **MET-1021 영구 결번**(되돌린 Stellite dup 의 stale freeze 매핑 제거 — 안정 ID 재사용 금지).) (R226b: 1198→1107; R226c: +7 cast → 1114; R226d: −2 dup → 1112; R226j: +1 ADC12 → 1113; R226l: +11 Granta → 1124; R226m: +5 → 1129.)
- **R226l~m Granta 파이프**: 사용자 제공 Granta MI PDF → RT 값 검증(교정: PEEK GF30 El·1015/1020 annealed·6150 El·Monel hard·**410 annealed 오염·304 1/4H·1/2H override 복원**) + **온도곡선 벡터 추출**(승인 — 결정적 좌표+**23°C 앵커 게이팅**: 실패 필드 제외) → `data/elevated-temp-curves.json` **by_id**(stable_id 키, build:data 가 부착·**datasheet 우선이라 파생 인라인 override**, 27 entry). 신규 entry 는 granta-datasheets.json(supRaw 합류, R226m +5: 304LN·316N·316FR·304 1/4H·1/2H). **유사재료(R226m)**: popularity 정렬·필터 폐기 → 물성 log-distance 순 top10, 타 인사이트그룹 후보 amber ↗배지. 인사이트 융합 `decisionContext`(ID 조회). Granta 웹 직접 수집 불가(라이선스 데스크톱 툴) — 사용자 PDF 제공만.
...
- **공정 가이드 ID 시스템** (R226j/C6): 절삭성·용접성 모델(ce/schaeffler/none)·HT family·선택 인사이트는 **런타임 name-regex 없이** `m.profiles`(빌드 스탬프) 조회. 분류는 `pnpm build:profiles`(scripts/build-process-profiles.mjs + lib/process-classify.mjs; HT 패턴 SSOT 는 ht-alloy-specific.ts 를 파싱) → `data/process-profile-assignments.json`(stable_id 키, 커밋). 교정은 `data/process-profile-overrides.json`(stable_id + src 필수). 콘텐츠 SSOT: `data/process-profiles.json`(프로파일 rating·note + 조건노트 `mach|htc` 26조합 + **R226r `condition_adjust`**(절삭성 열처리별 rating·cost 보정, ferrous 경화군 htc 배율 + `hardness_ref_hv` 로 경화클래스 내 경도편차 반영 — H900 vs H1150 등, 단단한 entry만 추가감산·부실경도는 class base 유지) + **`weld_condition_notes`**(경화/시효/냉간 HAZ 연화 노트))·`data/machining-guidance.json`(가공 가이드 21족)·`data/selection-insights.json`(when-to-use **19그룹**(+ceramic·composite)·109시나리오)·**`data/coating-recommendations.json`**(R226s/E12 — 후공정 추천 **22그룹** SSOT: 목적별 when·why·caution + 수소취성(UTS≥1000)/AM as-built/프로파일별 조건 보정; `m.profiles.cg` 스탬프, 카탈로그는 coatings.ts COATINGS(29종, substrateMatch regex 소멸), 게이트 `coating-recs.test.ts`). 게이트: build(전 entry 할당·키 parity·stale 검출) + `tests/process-profiles.test.ts` + `machinability-robustness`(가족-band 감사·회귀 앵커 8 + 조건보정 앵커). 클라이언트 해석: `lib/process-guidance.ts`(절삭성 rating 은 조성무관 base × htc 보정, 용접성 rating 은 조성기반 조건무관+HAZ 노트). 유사·대체 재료(`SimilarMaterialsCard`)는 상세 7 + compact 오버플로우(topN 16).
...
- **합금 개발배경(스토리)** (R226t/E13): SSOT `data/alloy-stories.json`(183종·403 entry) — 할당은 **stable_ids 동결**(구 name-매칭 3파일·build-materials 주입 은퇴, 레지스트리 story-free). build:data 가 stable_id 부착(`story`·`story_refs`·`story_key`·`story_v2`). v2 스키마(sections+timeline)·신뢰성 원칙·Opus 작문 회차 운영 → [`docs/STORY-SYSTEM.md`](docs/STORY-SYSTEM.md). 게이트 `tests/alloy-stories.test.ts`. **위키형 상호참조(재료↔재료·본문 용어 백링크)** (R227/E14/H) → [`docs/WIKI-CROSSREF-DESIGN.md`](docs/WIKI-CROSSREF-DESIGN.md) — 설계 A~M + **구현현황 §N·용어페이지 추가 플레이북 §O·개선 로드맵 §P**. 대부분 구현: 재료 백링크(공정탭 스토리 아래)·글로서리 SSOT `data/glossary.json`(101용어)·A4 상세 `data/glossary-articles.json`(101문서 — 전 용어 A4 완비, 표 스키마 `table{headers,rows}` 지원)·도표 `scripts/gen-glossary-figures.py`→`client/src/assets/glossary/*.png`(45종, matplotlib 3D/상태도, **Figure Quality v2 루브릭** — docs/GLOSSARY-IMPROVEMENT-PLAN.md §3.5)·용어페이지 `/guide/term/:slug`·본문 자동링크(`glossary-link.ts`, allowlist·KO경계). 용어 카운트는 `GLOSSARY.terms`에서 산출(하드코딩 금지). **품질 개선 계획·§3.1 열처리 용어 대응표·감사 3종**(audit-wiki-coverage·terminology·family-coverage → docs/audits/) → [`docs/GLOSSARY-IMPROVEMENT-PLAN.md`](docs/GLOSSARY-IMPROVEMENT-PLAN.md) (H4d Phase 0~7 완료). 게이트 `tests/glossary*.test.ts`·`wiki-link.test.ts`.
