...
- **원리**: 본문·스토리·노트에 **언급된 모든 재료·용어는 링크/정의**되어야 한다(1학년에게 미정의 전문어·죽은 언급 = 학습 실패). 언급 재료가 **DB에 있으면 링크, 없으면 신규 entry 추가**(§5.4) — "언급했는데 클릭할 데이터가 없음"을 근절.
...
- **4.1 `audit-wiki-coverage.mjs`** [D4·D9]: 전 article `body` + 스토리 `sections` + 절삭성/용접성/열처리 노트를 스캔해 재료·용어 언급을 추출하고 **3분류**한다:
  - **① 존재-미링크**: materials.json 이름·별칭과 매칭되나 링크 안 됨 → **백링크 배선**(예: 17-4 PH 스토리의 "PH 13-8"은 `R_0089` 존재하나 평문 → 링크만 걸면 됨).
  - **② 진짜 부재**: 어떤 entry·별칭과도 안 맞는 합금명 → **신규 DB entry 후보**(§5.4). 확인된 예: Custom 475·AM350·AM355·Custom 450. 스토리 183·403 entry 전수 스캔으로 전체 목록 도출.
  - **③ 미정의 용어**: 표준 lexicon에 있으나 글로서리 없는 전문어 → 신규 용어(§5·§6).
  > 규격 별칭 정규화 필수(UNS·EN·JIS·상용명) — 오탐(별칭이라 사실 존재) 방지. `aliasesByBase`·`shop-alias-dict` 재사용.
...
### 5.4 언급-부재 합금 → DB entry (사용자 지시) [D4·D9]
- **원리**: 카드·스토리·문서가 **언급하는 합금은 DB에 실물 entry로 존재**해야 한다 — 그래야 링크가 살고, "17-4 PH → PH 13-8" 같은 자매·비교 grade를 클릭해 볼 수 있다.
- **경로**: §4.1 감사의 **②진짜 부재** 목록이 입력. 각 부재 합금을 DATA-WORKFLOW로 신규 entry 추가(규격 datasheet 근거값·`tier` 지정·`related[]` cross-ref).
- **확인된 초기 후보(부재)**: Custom 475(Carpenter)·AM350·AM355(semi-austenitic PH)·Custom 450. 그 외 스토리 전수 스캔으로 확장(마레이징 M350/M250·특수 초합금·공구강 아종 등 예상).
- **주의**: "부재로 보이나 실은 존재"(별칭 미정규화)와 "진짜 부재"를 감사가 구분(§4.1 별칭 정규화). 존재-미링크(①)는 entry 추가가 아니라 **링크 배선**으로 해결 — 불필요한 중복 entry 생성 금지.
- **우선순위**: 스토리에서 **자주 언급되고 실무 대표성 있는** grade부터(등장 빈도×중요도). 값 신뢰성 최우선([[feedback_foreground_verification]] — 포그라운드 단일 검증).
...
| **3** | 백주철·ADI 글로서리+DB entry(§5) + **언급-부재 합금 DB 추가**(§5.4, 감사 ② 목록) + 계열 감사 후속 | D9·D4 |
