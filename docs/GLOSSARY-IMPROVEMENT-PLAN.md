...
### 3.5 Figure Quality v2 — 정보밀도 루브릭 (2026-07 사용자 피드백: "아직 너무 간단") [D1·D2 강화]
> 병인: "단순한 그림"의 뿌리는 **정보밀도 부족** — 형태만 있고 물리·수치·과정이 없다. 교과서 그림 수준이 목표: 한 장에서 개념·기구·수치 감각까지 얻을 수 있어야 한다. **전 도표(35+)에 소급 적용.**

한 도표가 갖춰야 할 층위(최소 3층):
1. **형태 층** — 대상의 사실적 개략(다각형 grain·실제 비례·음영/입체감).
2. **과정 층** — 무엇이 어떻게 변하는지(전/후 비교, 힘·흐름 화살표, 변형 영역 음영, grain flow).
3. **수치 층** — 대표 수치·치수·조건 주석(온도·압력·하중·치수 변화 t₀→t₁, 공식은 mathtext).
4. (권장) **인셋 층** — 확대경 인셋·미니 그래프(밀도-시간 곡선, 조직 확대)로 스케일 연결.

체크: 라벨-요소 겹침 0 · 콜아웃 리더 화살표 · 빈 공간 없이 구성(그러나 과밀 금지) · 3D/음영은 의미 있을 때(격자·입체 공정). 감사: 전 도표를 이 루브릭으로 재점검, 미달 도표는 회차별 업그레이드(§4.G 재활용 해소와 병행).
