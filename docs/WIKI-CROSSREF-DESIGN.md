...
> **2026-07 사용자 피드백 20건 반영 상세 실행 계획 → [GLOSSARY-IMPROVEMENT-PLAN.md](GLOSSARY-IMPROVEMENT-PLAN.md) (v2)**. 20 지적을 **10개 품질 차원**으로 일반화해 코퍼스 전역(74용어·42문서·183스토리·전 도표·생성 콘텐츠)에 적용하는 통찰형 계획 — 감사(audit)→표준→전역 교정. 백주철·ADI는 글로서리+DB entry(ASTM A532/A897). 용어표 확정. Phase 0~7. 아래는 요약.

- **분량**: A4 1장 → **2~3장**(기존 42문서 소급).
- **용어 영어원문 우선**: 담금질→Quenching·뜨임→Tempering·불림→Normalizing 등, 첫 등장만 "English(한글)". 스토리·절삭성·열처리·용접성 전역. **"정규화" 오역 수정 최우선**.
- **도표 전용화**: 재활용 최소(현재 grain-structure×6·steel-microstructures×5 등), 문서별 표제 전용 그림. 미세조직 = **Voronoi 불규칙 다각형** grain. martensite-morphology·ttt·steel-microstructures 개선 + austenite/cementite/carbide 전용 조직도 신규.
- **신규 용어**: austempering·metastable/stable-phase·precipitation·coarsening.
- **문서 보충**: 마르텐사이트(공구강·4340·마레이징 예시)·베이나이트(upper/lower·ADI)·시멘타이트(대폭 확장·백주철·크리프강)·탄화물(종류 표+조직도)·물성 글로서리(기계성질 7종).
- **절삭성·용접성**: 나열식 → 합금 맞춤 산문.
- **번호 항목 줄바꿈**·**백링크 확장**(surface_forms·authored `[[…]]`).
