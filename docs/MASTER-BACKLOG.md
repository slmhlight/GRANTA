...
| **H4b** | **용어 링크 배선 (스토리·detail·가이드 전역) + 팝오버** | ○ 다음 | A: **KO-aware 매처**(한글 조사경계 규칙표 — material 은 Latin 전용이라 회피했던 부분·신중)+GlossaryPopover(정의 1줄+용어 페이지 링크). `wiki-link` linkify 확장(용어 allowlist 합류). 스토리·detail text·가이드 본문 전역. R: 한글 과링크/오경계 / M: allowlist·섹션당첫등장·정밀도 표본 게이트 |
