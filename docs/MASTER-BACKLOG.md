...
| **H4** | **기술용어 글로서리 SSOT + 가이드 통합 렌더** | ✅ R227 | `data/glossary.json` **64용어**(8분류·display·surface_forms·autolink·short 정의·**출처 인용**·related, 동음이의 공식/공정 autolink 제외)+`glossary.test.ts`(6). `lib/glossary.ts`(번들 import)+`GlossaryBrowser`(가이드 챕터 "기술용어 사전", 검색·분류·관련칩). 본문 빈도조사(freq≥5 ~48종)로 범위 확정. |
| **H4b** | **기술용어 자동링크(가이드 본문·스토리→용어 페이지)** | ✅ R227 | `lib/glossary-link.ts` **KO-aware 매처**(라틴 \b·한글 단어시작경계+동음이의 가드 단조로운≠단조·최장우선·첫등장) + `glossary-link.test.ts`(12 정밀도). 가이드=`GlossaryText`(children 트리 재귀, F/코드/헤딩/링크 스킵·React 재렌더 안전). 스토리=`StoryLinkedText` 합성(재료 violet + 용어 teal, 정의 툴팁). 잔여: detail industry_note·추천 프로즈·GlossaryPopover(정의 팝오버) 확장 후보. |
| **H7** | **가이드 멀티페이지(사이드바·모바일 드로어·sub-TOC·용어 페이지)** | ✅ R227 | **사용자 지시**. `/guide`(랜딩)+`/guide/:section`(단일 챕터)+`/guide/term/:slug`(용어 상세). `Chapter` 라우트 인식(매칭만 렌더·하위호환). `GuideSidebar`(데스크톱 레일+모바일 드로어)·`ChapterSubToc`(h3 스캔)·`GuideTermPage`(정의·관련·출처)·`guide/toc.ts` SSOT. 랜딩 #chX 앵커 위임 SPA. 챕터 콘텐츠 무변경. |
| **H9** | **재료 상세 링크 네비 back-stack(버그수정)** | ✅ R227 | **사용자 지시**: 링크로 연 재료 닫으면 원래 재료도 닫히던 버그. Home `detailHistory` 스택+`onBack`(← 버튼)+**스마트 X**(이력 있으면 뒤로·루트만 닫기). |
| **H8** | **가이드 내 검색(위키스타일)** | ○ (H7) | E: `guide/index-entries.ts` 수동 인덱스(~36, chGloss 포함). A: 콘텐츠 파생/확장 인덱스 + 글로서리 용어 합류 + 사이드바 내 검색. |
