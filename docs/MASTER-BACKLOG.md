...
| A9 | **REGEX → Material ID 전면 전환** (빌드 파이프라인 name-regex override 소거) | ▶ 계획수립 R226o | 상세 계획 [REGEX-TO-ID-MIGRATION.md](REGEX-TO-ID-MIGRATION.md). Phase1(R199 값override→ID교정, 해법가 derivation順 보존)→2(R173/R205/R214)→3(메타)→4(인라인분류)→5(coatings 런타임)→6(aliasesFor). 매 Phase 값 불변(behavior identical) 수용조건. R: derivation stale·모놀리스 편집 / M: freeze 조회로 순서보존·값diff 게이트 |
