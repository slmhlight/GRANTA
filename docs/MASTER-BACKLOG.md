...
| E13 | **합금 개발배경(스토리) 시스템** — ID 이관·v2 골격·신뢰성 원칙 | ✅ 골격 R226t / ▶ 작문(Opus 회차) | 구 name-매칭(R75~R177, dead 17 유발) → **stable_id 동결 이관**(381 무손실 + dead 재연결 22 = 403). 단일 SSOT `data/alloy-stories.json`(183종) — 레거시 3파일·build-materials 주입 은퇴, 레지스트리 story-free(값/콘텐츠 분리). **v2 스키마**(hook/origin/breakthrough/adoption/today/fun_fact + timeline[ref]) + exemplar(inconel-718, 문단 무손실 재배치) + MaterialDetail v2 렌더러. 게이트 `alloy-stories.test.ts`(10: 스키마·sid 실재/중복·v2 구조·parity 전량·dead 허용목록·name-매칭 부재). 원칙·작문 가이드·우선순위 큐(T0 dead 7→T1 pop≥4→T2→T3, base 190→~300 목표): [STORY-SYSTEM.md](STORY-SYSTEM.md). R: 사실 오류 / M: 출처 규율(연도·인명 웹검증, v2 refs≥2)+DB 모순 시 값 교정 우선 |
