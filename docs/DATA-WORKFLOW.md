...
## 생성자(build-materials) 필요성 — 요약 (R226n 점검)
- **런타임/배포엔 불필요**: `build:data`(레지스트리→materials.json)만 씀. 890 name-regex override 는 live path 에서 은퇴.
- **신규 entry 추가엔 필요**: 소스 파일(granta-datasheets·cast-alloys·supplementary)을 레지스트리에 통합하려면 `build:registry`(=생성자+ID 할당). 레지스트리는 소스에서 재생성되는 derived 산출물이라 **레지스트리에 직접 쓴 entry 는 다음 재생성 때 소실** → 반드시 소스 파일에 둘 것.
- **오염 주의 → 근본 해결(R226n)**: 생성자의 `r199-stainless-overrides.json` name-regex 가 `"^AISI 304 "`(뒤 공백) 처럼 **조건 접미사까지 매칭**해 냉간가공/hard temper 신규 entry 의 datasheet 값을 annealed 로 덮던 문제(Monel hard·304 1/2H·1/4H) → **패턴에 `(?!.*(Hard|Cold-worked))` negative lookahead 추가**로 근본 차단. 신규 조건 grade 는 golden-values 앵커로 회귀 고정 권장.
