...
> §3.1 대응표 기준 한글 열처리어 사용처: **총 710 건 / 79 (용어×소스)**. Phase 5 영어원문 스윕의 작업 목록.
> **"정규화" 오역(열처리 문맥): 0 건** — 최우선 교정.

| 한글 패턴 | 영어 표준 | 소스 | 건수 | 오역 | 샘플 |
|---|---|---|---|---|---|
| 시효 | Aging | 글로서리 A4 (data/glossary-articles.json) | 71 |  |  강의 강화 원리가 바로 마르텐사이트(+후속 석출·시효)다." }, {  |
| 시효 | Aging | 스토리 (data/alloy-stories.json) | 43 |  |  어느 방향으로든 균일한 인성을 낸다. Aging(시효) 온도(H900~H1150)로 강도를 조율하는 것도  |
| 경화능 | Hardenability | 글로서리 A4 (data/glossary-articles.json) | 37 |  |  "heading": "임계냉각속도와 경화능", "figure": "ttt- |
| 담금질|소입 | Quenching | 스토리 (data/alloy-stories.json) | 31 |  | 프레드 빌름은 Al-Cu 합금을 Quenching(담금질)해 두고 며칠 뒤 강도가 두 배로 오른 것을 발견했 |
| 예민화 | Sensitization | 글로서리 A4 (data/glossary-articles.json) | 31 |  | 800 °C 에서 입계에 크로뮴 탄화물이 석출하는 예민화로 입계부식에 취약해질 수 있다(저탄소 L 등급·안정 |
| 뜨임|소려 | Tempering | 글로서리 A4 (data/glossary-articles.json) | 27 |  | 커서, 실제 부품에는 거의 항상 Tempering(뜨임)을 거쳐 인성을 회복시킨 뒤 사용한다."  |
| 시효 | Aging | 공정 프로파일 (data/process-profiles.json) | 21 |  | uCrZr 석출경화 Cu — 순동보다 훨씬 양호. 시효 상태 가공 권장.", "guidance_key |
| 담금질|소입 | Quenching | 글로서리 A4 (data/glossary-articles.json) | 20 |  | : "마르텐사이트는 강을 급랭(Quenching, 담금질)할 때 생기는 대표적인 경화 조직이다. 이름은 강의 |
...
| 시효 | Aging | 글로서리 정의 (data/glossary.json) | 16 |  |  "age hardening", "시효경화" ], "autolink |
...
| 담금질|소입 | Quenching | 글로서리 정의 (data/glossary.json) | 15 |  |  "short": "오스테나이트를 급랭(담금질)할 때 탄소가 빠져나가지 못하고 과포화된 체심정방( |
| 시효 | Aging | 선택 인사이트 (data/selection-insights.json) | 14 |  | 소 마르텐사이트라 용체화 상태 가공 → 480°C 시효(변형 ~0.06%)로 완성 — Q+T 강의 담금질 변 |
| 질화(?!물|텅스텐) | Nitriding | 글로서리 A4 (data/glossary-articles.json) | 13 |  | 기고, 반대로 표면을 매끄럽게 하거나 숏피닝·침탄·질화로 표면에 압축 잔류응력을 넣으면 수명이 크게 늘어난다 |
| 질화(?!물|텅스텐) | Nitriding | 스토리 (data/alloy-stories.json) | 12 |  | , 하드디스크 스핀들에 쓰인다. 초고속에서는 볼만 질화규소로 바꾼 하이브리드 베어링이 원심력·전식을 줄여 이 |
| 용체화 | Solution treatment | 글로서리 A4 (data/glossary-articles.json) | 12 |  |  "body": "시효(에이징)는 용체화·급랭으로 만든 과포화 고용체를 적절한 온도에서 유지 |
| 오스테나이트화 | Austenitizing | 글로서리 A4 (data/glossary-articles.json) | 12 |  |  "body": "Quenching(소입)은 오스테나이트화한 강을 임계냉각속도 이상으로 급랭해 마르텐사 |
...
| 뜨임|소려 | Tempering | 글로서리 정의 (data/glossary.json) | 10 |  | 어 생기는 상. 매우 단단하지만 취성이 커, 보통 뜨임으로 인성을 회복시킨다.", "source |
...
| 응력제거|응력 제거 | Stress relief | 글로서리 정의 (data/glossary.json) | 8 |  | lief": { "display": "응력제거풀림 (Stress Relief)",  |
| 오스템퍼링 | Austempering | 글로서리 A4 (data/glossary-articles.json) | 8 |  | 도를 고르게 한 뒤 서랭하여 변태 응력을 낮춘다. 오스템퍼링(austempering)은 아예 베이나이트로 변 |
...
| 경화능 | Hardenability | 글로서리 정의 (data/glossary.json) | 6 |  | 르텐사이트를 형성, 경화하는 열처리. 급랭 속도가 경화능을 좌우한다.", "sources": [ |
...
| 오스템퍼링 | Austempering | 글로서리 정의 (data/glossary.json) | 4 |  | ort": "구상흑연주철을 Austempering(오스템퍼링) 열처리해 ausferrite(침상 페라이트+탄 |
| 질화(?!물|텅스텐) | Nitriding | 글로서리 정의 (data/glossary.json) | 4 |  | /Acm 이상으로 가열 후 공랭하여 조직을 미세·균질화하는 열처리. 풀림보다 강도·경도가 약간 높다.",  |
| 풀림 | Annealing | 글로서리 A4 (data/glossary-articles.json) | 4 |  | 끊으면 된다. 잔류응력을 응력제거Annealing(풀림)으로 낮추고, 환경(염화물 농도·온도)을 관리하며,  |
| 마르퀜칭|마템퍼링 | Martempering | 글로서리 A4 (data/glossary-articles.json) | 4 |  | 열(quench crack)이나 변형이 생긴다 — 마르퀜칭 등으로 완화한다.\n② 잔류 오스테나이트는 치수  |
...
| 오스테나이트화 | Austenitizing | 글로서리 정의 (data/glossary.json) | 3 |  |  ], "short": "오스테나이트화한 강을 물·기름 등으로 급랭하여 마르텐사이트 |
