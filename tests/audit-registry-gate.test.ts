...
describe('audit:registry 게이트 (축2b)', () => {
  ...
  it('A6 역전 검사가 실제로 돌고 있다 (검사가 조용히 멈추는 것을 막는다)', () => {
    /* L 검사는 "역전 0" 이면 아무 말도 안 한다. 그런데 루프가 깨져 비교쌍이 0 이 돼도
       똑같이 조용하다 — 검사가 멈춘 것과 통과한 것이 구분되지 않는다. 그래서 **비교쌍 수**를
       본다. 지금 140 쌍이고, 크게 줄면 분류기나 범위 조건이 무너졌다는 뜻이다.
       오탐율도 함께 본다 — 백로그 A6 의 수용 기준이 '오탐 <5%' 였고 현재 1.4% 다. */
    const m = audit.out.match(/A6 역전 검사: 비교쌍 (\d+) · 역전 (\d+) \(([\d.]+)%\)/);
    expect(m, `'A6 역전 검사' 줄을 못 찾았다 — 검사가 빠졌는지 확인:\n${audit.out.slice(-400)}`).toBeTruthy();
    const [pairs, , pct] = [Number(m![1]), Number(m![2]), Number(m![3])];
    expect(pairs, `비교쌍이 ${pairs} 로 줄었다 — SOFT/HARD 분류나 제조형태 구분이 무너졌을 수 있다`).toBeGreaterThan(100);
    expect(pct, `역전 비율 ${pct}% — 수용 기준(5%)을 넘었다. 규칙이 과검출하거나 데이터가 나빠졌다`).toBeLessThan(5);
  });
