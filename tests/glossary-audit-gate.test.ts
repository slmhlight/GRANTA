...
describe('용어사전 감사 — 보류·필터가 낡지 않는다', () => {
  ...
  it('필터 계측이 돌고 있다 (과차단을 볼 수 있는 상태)', () => {
    /* 필터(STOP_RE·NON_MATERIAL·EN_STOP)의 0건은 보류와 뜻이 다르다 — 이쪽은 오탐 **부류**를
       막는 방어 가드라, 0건은 "지금 그 부류가 본문에 없다" 는 뜻이지 쓸모없다는 뜻이 아니다.
       지우면 연도·순숫자·냉각코드가 다시 합금명으로 잡힌다. 그래서 개수는 강제하지 않고,
       계측이 살아 있는지만 본다(과차단 조사 때 이 표가 입력이 된다). */
    expect(coverage, 'FILTER_AUDIT 출력이 없다 — 필터 계측이 꺼졌다').toMatch(/FILTER_AUDIT\(가드 미발화/);
  });
});
