...
const GOLDEN: G[] = [
  ...
  /* 'AISI 1010' / 'AISI 1025' 의 Anneal 앵커는 A3 철강 2027Q3 에서 삭제했다. bound 자체가 DB 의 합성값
     (158.4/281.6 · 198/303.6)에 맞춰 잡혀 있었고, 두 강종은 Modern Steels 데이터 페이지에도 ASM 탄소강
     대표물성 표(1015~1144)에도 소둔행이 없다 — 대조할 표준값이 없어 앵커가 성립하지 않는다. entry 는 제거. */
  ...
  /* 재캘리브레이션 — 구 bound(200~280 / 300~400)는 DB 의 합성 오염값 기준이었다.
     사다리에 물리 모순이 있어서 드러났다: 1015 소둔 UTS 345~430 인데 1030 이 300~400 이면
     탄소가 두 배인 강이 더 약하다는 뜻이 된다. 1015 는 R226l 에서 재보정했는데 1030 은 빠졌다.
     실제: YS 305~375 · TS 410~515 · El 25~38% · 120~145 HV (MatWeb 845°C 로냉 어닐 · Modulus Metal). */
  { name: 'AISI 1030', ht: 'Anneal', y: [305, 375], u: [410, 515] },
  ...
  { name: 'OFE Copper C10100', ht: 'Anneal', y: [55, 90], u: [200, 245], d: [8.9, 8.98] },
  ...
  /* W2-5(A3) Cu 족보 re-verify — copper.org(CDA) 대조로 잡은 오염을 앵커로 고정.
   * 어닐드 구리합금은 **연신율이 크다**는 것이 핵심 성질이라 el 앵커가 검출력을 갖는다. */
  { name: 'C26000', ht: 'Anneal', y: [85, 130], u: [300, 345], el: [55, 68] },   // OS050 103/324/62 — 세 템퍼 혼합 오염 차단
  { name: 'C17200 —', ht: 'Anneal', y: [130, 200], u: [440, 520], el: [30, 45], d: [8.2, 8.32] },  // TB00 — 시효값(1100/1300) 오기 차단
  { name: 'Phosphor Bronze C51000', y: [120, 165], u: [315, 360], el: [48, 66] },  // OS035 138/338/58
  { name: 'C95400 Aluminum Bronze', y: [195, 260], u: [505, 600], el: [10, 16] },  // B148 M01 min 207/517
  { name: 'Copper (Pure, C11000)', ht: 'Anneal', y: [55, 90], u: [200, 245], el: [40, 52], d: [8.9, 8.98] },
