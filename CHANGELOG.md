...
대조는 **Modern Steels and Their Properties**(Bethlehem Steel Handbook 2757)의 grade 별 *단일 heat* MASS EFFECT 표 —
`Annealed / Normalized / 담금질 + 뜨임온도` 행을 1/2·1·2·4 in. 봉 단면별로 싣는다. 교차확인은 **EMJ Mechanical Properties
and Hardenability**(Sec. R)와 **Shigley Table A-21**(ASM Metals Reference Book 2d ed.). 판정 대장은 `docs/audits/steel-reverify-2027Q3.md`.

**합성의 지문 — 세 물성이 같은 배수로 움직였다**
조건별 값이 `ys·uts·경도` 세 물성에서 **정확히 같은 비율**을 가졌다(1010 0.87129 · 1025 0.87118 · 4150 0.81481/0.93519 ·
5130 0.94444/1.15909 · 5140 0.93519). 서로 다른 열처리가 세 물성을 같은 배수로 올리는 일은 없다 — 표에서 옮긴 값이 아니라
한 줄에 배율을 곱해 찍은 값이라는 뜻이다(CSV generic 파이프의 잔재). 곁들여 드러난 것: **1010 Q+T 와 1015 Q+T 의 경도가
95.95 HB 로 완전히 같았고**(탄소량이 다른 두 강이 같을 수 없다), **1030 Q+T(262.6/383.8)가 자기 소둔행(341/464)보다 약했다**
(담금질·뜨임재가 소둔재보다 무를 수는 없다), **1050·1080·1095 Q+T 가 ys/uts 0.70 · HB/uts 0.295 라는 같은 고정비**를 공유했다.

**교정 13 — 네 물성을 같은 행에서 가져온다 (1 in. 봉 · 1000 °F(540 °C) 뜨임)**
- 소둔 5: **1040 272.8/497.2 → 353/519**(uts/ys 비 1.82 는 소둔 0.40 %C 강에서 불가) · **6150 항복 480 → 412**(인장·연신·경도는 맞는데
  항복만 17 % 높았다 — 소둔재 항복은 허용응력의 기준이라 **안전쪽이 아니라 위험쪽** 오차) · **4150 704/968 → 379/729** ·
  **5140 연신 22 → 28.6 · 경도 170 → 167** · **1015 경도 83.6 → 111**(1010 소둔과 같은 값이던 자리).
- Q+T 8(1030·1040·1050·1080·1095·4150·5140·6150): 뜨임온도·단면이 없는 "Quenched / tempered" 는 같은 강에서 항복이 두 배 넘게
  갈리는 구간을 한 줄로 뭉갠 표기다. 값을 원전의 540 °C 뜨임행으로 바꾸면서 **이름·열처리 표기도 `Q+T (540°C)` 로** 바꿔
  값과 라벨이 같은 것을 가리키게 했다(4130 `Q+T (550°C)` 와 같은 형식). 같은 base 의 나머지 행은 `(Wrought)` 접미를 떼어 사다리 통일(14).
- 딸린 정리: **6150 Q+T 피로 760 → 598 MPa**. 계열 밴드가 스프링용 저온뜨임(인장 1700 MPa 대) 기준이라, 인장을 1196 으로
  교정하자 강재 내구한도 상한(0.63·UTS = 753)을 넘었다. 이 조건의 공표값이 없어 Shigley Se′ ≈ 0.5·Sut 로 R214 경로에
  `derived`·`estimated` 로 정직 표기.

**제거 12 — 교정하려면 옮겨올 행이 있어야 한다**
`1010 소둔·Q+T` · `1015 Q+T` · `1020 Q+T` · `1025 소둔·Q+T` · `5130 4조건 전부` · `4150 냉간인발` · `5140 냉간인발`.
**1010·1025·5130 은 Modern Steels 데이터 페이지에도 ASM 대표물성 표에도 행 자체가 없다** — 값을 남기려면 지어내야 한다.
4150·5140 냉간인발은 그 강종의 제품형이 아니다. 사유·날짜·대체 참조는 `data/corrections/remove.json` 의 `reasons` 에 적었고,
옛 딥링크(`?d=<옛 id>`)는 제거 원장을 통해 "언제·왜 제거됐는지" 를 안내한다. 재료 **1,071 → 1,059**.

**D9 종결 — 물리 역전 공개 목록이 비었다**
W2-2 가 남긴 8 base(1010·1015·1020·1025·1030·1040·1050·4150)의 "강화됐는데 연신율도 올랐다" 는 전부 같은 원인이었다.
교정·제거 후 8 base 모두 역전이 사라져 `tests/physical-ordering.test.ts` 의 `KNOWN_OPEN` 을 **비우고 상한을 8 → 0** 으로 내렸다 —
이제 같은 유형의 재발은 곧바로 게이트 실패가 된다. 마지막 잔여였던 **AISI 1025 소둔**은 교정이 아니라 제거로 닫혔다
(2026-09-11 재조사가 이미 "표준표에 1025 행 자체가 없다"고 결론냈다면, 남겨 둘 근거도 없다).

**D04 마감 — 규격 하한은 "수치가 가깝다" 가 아니라 "원문이 그렇게 말한다"**
`data/spec-floor-declarations.json` 신설: **사람이 원문을 대조한 것만** `basis='min_spec'` 이 된다(`std`·`note`·`verified` 전부 필수).
1차 선언 **13 entry / 21 필드**(A36 3 · A572 Gr50 2 · A588 GrA · A992 · A516 Gr70 · API 5L X42N/X70/X80 · A216 WCB/WCC).
나머지는 `min_spec_value`·`min_spec_source` 별도 축(266 필드)에 병기하고 근접은 `near_min_spec` 표시로만 쓴다(78).
곁들여 **A516 Grade 70** 의 인장 범위 하한 460 MPa 이 규격 대역(485–620) 밖이던 것을 대역으로 맞췄다.
게이트(`spec-floor`·`audit-deep`)는 "floor 배지는 원문 대조 기록 또는 교정 인용 중 하나를 반드시 갖는다"를 강제한다.

**그 밖**
- **`stable_id` 를 산출물에 싣는다** — 이름·조건이 바뀌어도 변하지 않는 공개 식별자(materials.json · slim index · CSV `Stable ID` 열).
  이번처럼 이름이 바뀌는 교정에서 CSV·보고서·제거 원장이 같은 재료를 가리키려면 이 키가 필요하다.
- **golden 앵커 2개 삭제**(`AISI 1010 [Anneal]` · `AISI 1025 [Anneal]`) — 두 밴드는 DB 의 합성값에 맞춰 잡혀 있었다.
  대조할 표준값이 없으면 앵커 자체가 성립하지 않는다.
- 제거된 12 ID 를 가리키던 상류 설정 정리(값 교정 2 · r199-stainless 7 · r199-source-urls 12 · r205-reliability 16 ·
  r173-handbook-sources 3 · 부식 노트 1 · 스토리 멤버 12) — 전부 **게이트가 먼저 잡아낸** 것이다.
- 검증: vitest 1367/1367(88) · tsc 0 · lint 0 · anomaly 0(high/med/low) · 라운드트립 불일치 0 · audit:registry 0 · audit:all 전지표 0.

---

## 2026-09-22 — 감사 3차 심층보고서(AUD-3) 대응: 의미가 과도하게 확정되던 경로 4개(D01·D04·D05·D06)와 소비 경로 4개(D02·D03·D07·D08)

같은 날 받은 3차(심층 보강본) 보고서의 9개 문제군을 닫았다. 공통 주제는 **"데이터가 말할 수 있는 것보다 더 많이 말하고 있었다"**.
게이트 `tests/audit-deep-2026-09-22.test.ts`(21) · `tests/partial-data-notice.test.tsx`(3), 기존 `spec-floor`·`unexposed-data` 계약 개정.

**의미를 과도하게 확정하던 경로**
- **D01 값 교체와 계보의 원자성** — 값을 덮는 override 가 "그 값이 어떻게 나왔는지" 설명하던 provenance 를 남겨, 피로 9건이 `handbook` 배지 아래 옛 HT 계수식(`alloy:1020 × HT:cold worked (f×1.25…)`)을 달고 있었다. 계열 폴백만 갈아 끼우던 규칙을 **모델 계산 provenance**(× HT: · σf≈ · 재유도)까지 확장하고, override 의 근거 문장에서 규격·제조사 인용을 찾아 올린다(`SAE J404`·`ASM Vol.1` 등, 없으면 근거 문장 자체를 싣고 개발 서사는 sanitize). KIC 모델에도 `model` 스탬프 — 이제 `× HT:` 가 남은 값은 반드시 기초값·계수를 갖는다(게이트).
- **D04 규격 하한 자동 지정 중단** — "typical 이 표의 minimum ±2% 이면 이 값이 곧 규격 보증 최소값"이라는 **수치 근접 규칙**을 없앴다(101 필드). 규격 최소값은 별도 축(`min_spec_value`·`min_spec_source`, 330 필드)에 병기하고, 근접은 `near_min_spec` 표시로만 남겨 상세가 "표시값이 규격 최소와 2% 이내 — 대표값인지 하한인지 출처에서 확인" 이라고 말한다. `basis='min_spec'` 은 교정이 직접 선언한 것만(3). 보고서가 지목한 규격 불일치 2건도 규격 숫자로 교정: **Ti-6Al-4V Gr23 ELI σy 790 → 795**(ASTM F136 115 ksi) · **API 5L X70 UTS 565 → 570**(PSL2 L485).
- **D05 규제 자료 부족 ≠ 통과** — Pb/Cd/Hg 를 0 으로 시작해 **조성에 그 원소가 없어도** "RoHS 통과"로 저장하던 것(1,041 entry, 그중 42 는 조성 자체가 빈 객체)을 3-상태로: 세 원소 중 하나라도 기재돼야 한계 비교(true/false), 하나도 없으면 `null`(자료 부족) + `rohs_basis` 근거. 필터는 "RoHS **확인 적합**만"(true 만)으로 바뀌고 라벨이 "미기재는 자료 부족으로 제외 — 부적합 아님" 을 밝힌다. 상세에는 판정하지 않은 이유(균질재료·적용범위·면제 포함이라 제조사 선언 필요)를 적는다. 현재 확인 적합 26 · 초과 2 · 자료 부족 1,043.
- **D06 분포점과 범위의 정합** — points↔ranges 허용오차(절대 0.51·상대 2%)가 너무 커서 밀도는 사실상 무검사였고 4340 소둔 σy(점 540 vs 범위 415–530)가 1.9% 로 통과했다. 1% 상대 + 물성별 바닥(밀도 0.02)으로 조여 재동기 99 → 118 entry. **CPM 3V 밀도**는 점 7.46 / 범위 7.85 가 갈라져 있었는데 Crucible 데이터시트(0.28 lb/in³ = 7.80)로 통일했다.

**소비 경로**
- **D02 로딩 상태와 검색의 계약** — slim 인덱스에는 대표값만 있고 cp·poisson·tmelt 는 아예 없어 샤드 전후로 결과가 갈렸다(cp>500 0→586 · yield>500 386→402). 전량 필요 판정을 문자열 접두사에서 **파싱된 제약**으로 바꿔(수치 제약은 상·하한을 읽으므로 전부 해당) 즉시 전 샤드를 요청하고, 도착 전에는 "이 결과는 아직 확정이 아니다" 를 고지한다(샤드 실패 시 카테고리명 + 재시도).
- **D03 경도 척도 보존** — 환산표(E140) 밖이라 원 스케일로 남긴 20 건(HB)이 CSV 의 `Hardness (HV)` 열·HV 수치 검색·Ashby 경도 축에서 HV 로 취급됐다. 척도를 slim 인덱스에 싣고(`hardness_scale`), 공용 판정 `comparableOnHV` 로 수치 검색·차트에서 제외하며, CSV 는 값 열 + **척도 열**로 나눈다(환산표 밖 값을 억지로 환산하지 않는다).
- **D07 검사와 배포의 연결** — CI 와 Deploy 가 각각 push 에 반응해 서로를 기다리지 않았다. 배포 워크플로 안에 `verify` job(build:data·wiki·stats → check → lint → test)을 두고 `build` 가 `needs: verify` 로 기다린다. 게이트는 `pipeline-integrity`.
- **D08 화학식 표기** — `B4C` 검색이 순수 `Boron Carbide (B₄C)` 를 놓쳤다(19 재료·22 토큰). 검색 키에 NFKC 정규화를 적용해 표시 이름은 그대로 두고 아래첨자·ASCII 를 같은 키로 접는다(B4C 검색 = 복합재 2 + 순수 세라믹 1).

**그 밖**
- **F11 잔여** — 링크 상태 배지가 `dead` 만 보이던 것을 **접근 보류**(자동 요청 차단·요청 실패·미검사)까지 구별해 표시. 가이드 ch9 의 American Elements 3D printing materials(반복 404) → EOS Metal Materials · Nikon SLM Materials.
- **F23 잔여** — 상세의 generic 경고·레이더 설명·신뢰도 범례·A/B basis 링크, Compare 의 막대 툴팁·내보내기 오류 문구 EN.
- **Q03** — CSV 에 `ID`·`Stable ID` 열(같은 이름의 다른 조건·다른 배포 구분).
- 검증: vitest 1369/1369(88) · tsc 0 · lint 0 · anomaly 0 · 라운드트립 0.
