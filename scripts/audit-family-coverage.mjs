...
const SUBCAT_TO_SLUG = {
  ...
  // 나머지 subcat 은 의도적으로 미매핑 → (a) 리포트에 후보로 등장
};
...
const uncovered = rows.filter(([sc]) => !SUBCAT_TO_SLUG[sc.split(' / ')[1]]);
console.log(`audit-family-coverage: subcat ${bySubcat.size} · alloy-family 용어 ${famTerms.length} · 미커버 subcat ${uncovered.length}`);
console.log('미커버 상위:', uncovered.slice(0, 10).map(([sc, l]) => `${sc.split(' / ')[1]}(${l.length})`).join(' · '));
