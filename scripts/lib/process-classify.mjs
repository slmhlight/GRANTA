...
const METAL_MACH = [
  ...
  // R226v — 청동은 al-* fallback("aluminum" 단어) 앞에서 판정 (Aluminum Bronze 오분류 방지)
  [/bearing bronze|c93[0-9]{3}/i, 'bronze-bearing'],
  [/phosphor bronze|c5[01]\d{3}|silicon bronze|c65[15]00|alumini?um bronze|c6[13]\d{3}|c95[48]\d{2}/i, 'bronze-wrought'],
  ...
  [/brass|c[34]\d{4}|c2[28]\d{3}|muntz|commercial bronze/i, 'brass'],
  [/cupronickel|c70600|c71500|cuni\s?30|nickel silver|c75200/i, 'cuni'],
  [/beryllium|c17200|\bbecu\b|cube2|be-?cu\b/i, 'becu'],
  [/copper.*pure|ofhc|c11000|c10100|c10200|c12200|c10300|c12000|\bdlp\b|\bdhp\b/i, 'cu-pure'],
  [/copper.*cr.?zr|cucrzr|cuni2sicr|c18000|c1815\d|c18200|grcop|narloy|c19400/i, 'cu-crzr'],
  [/\b1018\b|\b1020\b|carbon steel.*low|sae 10[12]\d|\b10(08|10|15|16|22|25)\b/i, 'carbon-low'],
  // R226v — 저탄소 구조·판재·조선·라인파이프·한국 KS 강 (분류 근거: 조성 저탄소 C-Mn — carbon-low 급)
  [/\ba36\b|\ba57[02]\b|\ba99[26]\b|\ba51[67]\b|\ba537\b|sa51[67]|\bs27[05]j?r?\b|\bs355j?\d?\b|\bs690q?l?\b|\bah36\b|\bdh36\b|\beh36\b|api\s?(5l|2h)|line pipe|psl\s?2|\bwc[bc]\b/i, 'carbon-low'],
  ...
  [/zircaloy|zirconium|\br60[78]\d{2}\b/i, 'zirconium'],   // R226v — Zr 은 Ti 유사 절삭 (구 refractory 18% 과대 교정)
  [/tungsten/i, 'tungsten'],
  [/tantalum|niobium/i, 'ta-nb'],
  [/molybdenum|tzm|mo-?la|moly/i, 'moly'],
  [/refractory/i, 'refractory'],
  // R226v — 신규 클래스 (배열 끝: 위 기존·확장 패턴이 못 잡은 잔여 지정자 전용)
  [/invar|kovar|fe-?ni36|controlled expansion/i, 'ni-fe-lowexp'],
  [/hadfield|\bmn13\b|x120mn12|manganese steel/i, 'hadfield'],
  [/gr[ae]y (cast )?iron/i, 'cast-iron-gray'],
  [/ductile iron|nodular|\ba536\b|ggg\d{2}/i, 'cast-iron-ductile'],
  [/zamak|\bza-?\d{1,2}\b|zinc/i, 'zinc-diecast'],
];
