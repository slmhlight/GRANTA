...
const REAL_PROPS_ELEV_SRC_DEFAULT = '핸드북 typical — 개별 인용 미기재';
...
const ELEV_SRC_BATCHES = [
  ...
  [['ti6al4v', 'ti-6al-4v', '304l', '316l', '7075', '6061', 'h13', 'cu (pure', 'waspaloy', 'rene 41'],
    'MMPDS-15 · ASM Handbook Vol.2 · Special Metals · Carpenter Technology datasheets'],
  [['cocrmo', 'l605', 'mar-m247', '17-4 ph', '17-4ph', '15-5 ph', '15-5ph', 'a286', 'a-286', '4140', 'nimonic 80a', 'inconel 706', 'inconel 600'],
    'ASTM F75/F90 · ASM Handbook Vol.2 · Special Metals SMC · Haynes International H-3068C · Carpenter datasheets'],
  ...
  [['peek victrex 450g', 'peek victrex 450ca30', 'peek solvay ketaspire', 'ultem 1010', 'ultem 9085', 'pekk kepstan',
    ...
    'Victrex PEEK technical guide · SABIC ULTEM design guide · Solvay KetaSpire/KEPSTAN datasheet · DuPont Delrin handbook · SABIC Lexan design guide'],
...
/* R75/R78 → R226t/E13 — 스토리 name-매칭 주입 **은퇴**.
 *   구 3파일(material-stories*.json)의 exact/base/prefix 이름 매칭은 rename 에 취약(dead 17 발생) →
 *   381 매칭을 stable_id 로 동결 이관한 data/alloy-stories.json 이 단일 SSOT.
 *   부착은 build-from-registry(build:data)가 stable_id 로 수행 — 레지스트리는 story-free (콘텐츠/값 분리). */
...
try {
  ...
  for (const [names, src] of [
    ...
    [[...Object.keys(r150Raw.composites || {}), ...Object.keys(r150Raw.polymers || {})], '제조사 datasheet 측정값'],
    [Object.keys(r151Raw.polymers || {}), 'Victrex·Solvay·Celanese·Arkema·SABIC·BASF·Stratasys datasheet 측정값'],
...
try {
  ...
  for (const patch of patches) {
    const targets = matchByStableIds(patch);   // R226p — stable_id 매칭 (구 RegExp(patch.pattern,'i'))
    for (const m of targets) {
      r173Touched++;
...
try {
  ...
  const patterns = r191Raw.patterns || [];
  // R226p — 캡처 시 각 pattern 의 stable_id 집합 기록('r191:' 키). 라이브는 stableIds 로 first-match.
  if (R226P_CAPTURE) for (const p of patterns) {
    const re = new RegExp(p._namePattern_ref || p.pattern, 'i');
    R226P_captured['r191:' + (p._namePattern_ref || p.pattern)] = [...new Set(all.filter(m => m && m.name && re.test(m.name)).map(sidOf).filter(Boolean))].sort();
  }
  const patSids = patterns.map(p => new Set(p.stableIds || []));
  let r191Touched = 0;
  const r191Hits = {};
  for (const m of all) {
    ...
    for (let i = 0; i < patterns.length; i++) {
      if (patSids[i].has(sid)) {   // R226p — stable_id first-match (구 regex first-match)
        const p = patterns[i];
        m.manufacturers = [p.manufacturer];
        m.manufacturer = p.manufacturer;
        r191Touched++;
        r191Hits[p._alloy || p._namePattern_ref || p.pattern] = (r191Hits[p._alloy || p._namePattern_ref || p.pattern] || 0) + 1;
        break;
      }
    }
  }
