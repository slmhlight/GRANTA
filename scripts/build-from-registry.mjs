...
// 1b) 출처 라벨 정리 (R226d) — placeholder 라벨("Datasheet N"·"MatWeb N", URL 은 실재) → URL 도메인 기반 서술 라벨.
//     url 필드에 표준 인용(비-http 텍스트)이 든 경우 그 텍스트를 라벨로 승격(+ 가짜 url 제거). 값 SSOT(레지스트리)는 불변 — presentation 도출.
const PUBLISHER = {
...
// 2) anomaly 재검출 (build-materials.mjs detectAnomalies 와 동일 로직 — cutover 후 이쪽이 canonical)
  ...
  for (const m of all) {
    ...
    if (sy != null && uts != null && uts > 0 && !hasVerifiedSource) {
      ...
      if (cat === 'Metal') {
        ...
        else if (/Stainless Steel - Austenitic/.test(sub) && (ratio < 0.20 || ratio > 0.95)) push('med', 'Austenitic SS σy/UTS out of [0.20, 0.95]', m, `${ratio.toFixed(2)}`);
        else if (/Carbon Steel/.test(sub) && (ratio < 0.35 || ratio > 0.95)) push('low', 'Carbon Steel σy/UTS out of [0.35, 0.95]', m, `${ratio.toFixed(2)}`);
        else if (/Nickel Superalloy/.test(sub) && ratio > 0.99) push('med', 'Ni Superalloy σy/UTS > 0.99 (suspect)', m, `${ratio.toFixed(2)}`);
        else if (/Maraging/.test(sub) && ratio > 1.00) push('low', 'Maraging σy/UTS > 1.00', m, `${ratio.toFixed(2)}`);
        else if (/Titanium - α\+β/.test(sub) && (ratio < 0.75 || ratio > 0.99)) push('low', 'Ti α+β σy/UTS out of [0.75, 0.99]', m, `${ratio.toFixed(2)}`);
        else if (/Tool Steel/.test(sub) && ratio > 0.99) push('low', 'Tool Steel σy/UTS > 0.99', m, `${ratio.toFixed(2)}`);
      }
